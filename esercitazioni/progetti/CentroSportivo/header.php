<header>
    <nav>
        <div class='left'>
            <img src="./resources/img/FootLab_AS_Logo.png" alt="stemma">
            <ul>
            <?php
                /**
                ...
                $navMenu = json_decode(file_get_contents('./databases/navMenu.json'), true);
                foreach ($navMenu as $key => $item) {
                    ...
                    if($item['type'] == 'nav_team'){
                        if($UserLogged && $user->get_myTeam_name() != null){
                            if($selected == $item['id']){
                                echo "<li><a href='{$item['link']}?selezione={$item['id']}' class='{$item['type']}' id='selected'>{$item['nome']}</a></li>";
                            } else {
                                echo "<li><a href='{$item['link']}?selezione={$item['id']}' class='{$item['type']}'>{$item['nome']}</a></li>";
